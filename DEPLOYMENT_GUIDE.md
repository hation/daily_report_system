...
1. 检查 `src/processors/data_analyzer.py` 中的关键词匹配规则是否需要调整
2. 验证每日摘要是否包含关键主题和项目信息
3. 查看关键产出是否正确识别完成、决策和进展类型

### 报告格式问题

如果报告显示格式不正确：

1. 检查 Markdown 渲染引擎是否支持所需的格式
2. 验证表格对齐和换行是否符合 Markdown 规范
3. 确认链接是否正确格式化为 `[text](url)` 样式
